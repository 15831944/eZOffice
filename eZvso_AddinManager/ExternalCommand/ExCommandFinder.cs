...
namespace eZvso.AddinManager
{
    ...
    {
        /// <summary> 将程序集文件加载到内存，并且提取出其中的 Visio 外部命令 </summary>
        /// <param name="assemblyPath"></param>
        /// <returns></returns>
        public static List<IVisioExCommand> RetriveExternalCommandsFromAssembly(string assemblyPath)
        {
            Assembly asm;
            ...
            // 方法二：通过LoadFile加载，在测试中发现如果这样做，则在 eZvso_AddinManager 调试过程中，如果在Visual Studio中修改了代码，则不能重新进行编译。
            //asm = Assembly.LoadFile(assemblyPath);

            return GetExternalCommandClass(asm);
        }

        private static List<IVisioExCommand> GetExternalCommandClass(Assembly ass)
        {
            List<IVisioExCommand> ecClasses = new List<IVisioExCommand>();
            Type[] classes = null;
            ...
            if (classes != null)
            {
                ...
                {
                    if ((cls != null) && cls.GetInterfaces().Any(r => r == typeof(IVisioExCommand)))
                    // 说明这个类实现了 Visio 的命令接口
                    {
                        ...
                        if (m != null && m.IsPublic)
                        {
                            ...
                            IVisioExCommand exC = ins as IVisioExCommand;
