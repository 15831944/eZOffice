...
namespace eZvso.AddinManager
{
    ...
    {
        private static IVisioExCommand _currentExternalCommand;
        private static string _currentExternalCommandAssemblyPath;

        /// <summary> 执行当前（即上次执行过的那个）外部命令 </summary>
        /// <param name="VisioApp">作为Execute()方法的输入参数的对象，表示当前的Visio Application </param>
        public static void InvokeCurrentExternalCommand(Application VisioApp)
        {
            if (_currentExternalCommand != null && _currentExternalCommandAssemblyPath != null)
            {
                InvokeExternalCommand(_currentExternalCommandAssemblyPath, _currentExternalCommand, VisioApp);
            }
        }

        /// <summary> 执行 Visio 的外部命令 </summary>
        ...
        public static void InvokeExternalCommand(string assemblyPath, IVisioExCommand externalCommand, Application VisioApp)
        {

            ExCommandExecutor.RunActiveCommand(externalCommand, assemblyPath, VisioApp);
            //
        ...
        private static void InvokeCommand(MethodInfo externalCommandMethod, Application VisioApp)
        {
            // 创建一个实例对象
            ...
            object[] parameters = new object[] { VisioApp, "", errorObj };
            try
            {
                // 执行操作
            ...
            switch (res)
            {
                ...
                case ExternalCommandResult.Cancelled:
                    {
                        // 由于没有在Visio中没有事务或者回滚，所以直接结束就可以了。
                        break;
                    }
        ...
        private static ExternalCommandResult RunActiveCommand(IVisioExCommand addinItem, string assemblyPath, Application VisioApp)
        {
            string errorMessage = "";
            object errorObj = null;
            ...
            try
            {
                ...
                else
                {
                    ...
                    IVisioExCommand newExCommand = assembly.CreateInstance(addinItem.GetType().FullName) as IVisioExCommand;
                    ...
                    {
                        result = Execute(newExCommand, VisioApp, ref errorMessage, ref errorObj);// 如果在 Execute() 中发现某个程序集不存在，则通过AssemblyResolve 事件手动进行加载
                    }
                }
            }
            catch (Exception ex)
        ...
        private static ExternalCommandResult Execute(IVisioExCommand exCommand, Application VisioApp, ref string errorMessage,
            ref object errorObj)
        {
            ...
            try
            {
                ...
                res = exCommand.Execute(visioApp: VisioApp, errorMessage: ref errorMessage, errorObj: ref errorObj);
            }
            catch (Exception ex)
            {
                if (string.IsNullOrEmpty(errorMessage))
