...
namespace eZx.AddinManager
{
    ...
    public static class ExCommandFinder
    {
        ...
        private static List<IExcelExCommand> GetExternalCommandClass(Assembly ass)
        {
            List<IExcelExCommand> ecClasses = new List<IExcelExCommand>();
            Type[] classes = null;
                ...
                foreach (Type cls in classes)
                {
                        ...
                        // 说明这个类实现了 Excel 的命令接口
                    {
                        ...
                        if (m != null && m.IsPublic)
                        {
                            ...
                            IExcelExCommand exC = ins as IExcelExCommand;
