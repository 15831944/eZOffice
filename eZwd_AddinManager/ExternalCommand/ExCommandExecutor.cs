...
namespace eZwd.AddinManager
{
    ...
    {
        private static IWordExCommand _currentExternalCommand;
        private static string _currentExternalCommandAssemblyPath;

        /// <summary> 执行当前（即上次执行过的那个）外部命令 </summary>
        /// <param name="WordApp">作为Execute()方法的输入参数的对象，表示当前的Word Application </param>
        public static void InvokeCurrentExternalCommand(Application WordApp)
        {
            if (_currentExternalCommand != null && _currentExternalCommandAssemblyPath != null)
            {
                InvokeExternalCommand(_currentExternalCommandAssemblyPath, _currentExternalCommand, WordApp);
            }
        }

        /// <summary> 执行 Word 的外部命令 </summary>
        ...
        public static void InvokeExternalCommand(string assemblyPath, IWordExCommand externalCommand, Application WordApp)
        {

            ExCommandExecutor.RunActiveCommand(externalCommand, assemblyPath, WordApp);
            //
        ...
        private static void InvokeCommand(MethodInfo externalCommandMethod, Application WordApp)
        {
            // 创建一个实例对象
            ...
            object[] parameters = new object[] { WordApp, "", errorRange };
            try
            {
                // 执行操作
        ...
        private static ExternalCommandResult RunActiveCommand(IWordExCommand addinItem, string assemblyPath, Application WordApp)
        {
            string errorMessage = "";
            ...
            try
            {
                ...
                else
                {
                    ...
                    IWordExCommand newExCommand = assembly.CreateInstance(addinItem.GetType().FullName) as IWordExCommand;
                    ...
                    {
                        result = Execute(newExCommand, WordApp, ref errorMessage, ref errorObj);// 如果在 Execute() 中发现某个程序集不存在，则通过AssemblyResolve 事件手动进行加载
                    }
                }
            }
            catch (Exception ex)
        ...
        private static ExternalCommandResult Execute(IWordExCommand exCommand, Application WordApp, ref string errorMessage,
            ref object errorObj)
        {
            ...
            try
            {
                ...
                res = exCommand.Execute(wdApp: WordApp, errorMessage: ref errorMessage, errorObj: ref errorObj);
            }
            catch (Exception ex)
            {
                if (string.IsNullOrEmpty(errorMessage))
